...
class AppState extends ChangeNotifier {
  ...
  Future<void> signInWithGoogle() async {
    try {
      ...
      // Download dados da nuvem após login
  ...
  List<AtividadeRecarga> get recargasHoje {
    final hoje = dataChave;
    return boxRecargas.values
        .where((r) {
          final ts = r.timestamp;
          return DateFormat('yyyy-MM-dd').format(ts) == hoje;
        })
        .toList();
  }

  Future<void> adicionarRecarga(AtividadeRecarga recarga) async {
    await boxRecargas.add(recarga);
    notifyListeners();
  }
